...
app.post('/login', (req, res) => {
  ...
  forwardRequest(req, res, PROFILE_SERVICE_URL, '/login');
});
...
app.put('/users/:id', (req, res) => {
  ...
  if (!name && !email && !password) {
    return res.status(400).json({ 
      error: 'At least one field (name, email, password) must be provided' 
    });
  }

  console.log(`✏️ User update request for ID ${userId}:`, {
    name: name || 'not provided',
    email: email || 'not provided',
    password: password ? '***' : 'not provided'
  });

  forwardRequest(req, res, PROFILE_SERVICE_URL, `/users/${userId}`);
});
...
// === TEST ENDPOINT FOR PROFILE SERVICE ===
app.post('/test/profile', async (req, res) => {
  ...
  try {
    ...
    const registerResponse = await axios.post(`${PROFILE_SERVICE_URL}/users`, testUserData, {
      headers: {
        'Content-Type': 'application/json'
      },
      timeout: 10000
    ...
    // Test user login
    const loginResponse = await axios.post(`${PROFILE_SERVICE_URL}/login`, {
    ...
    res.json({
      message: 'Profile service test successful',
      registration: registerResponse.data,
      login: loginResponse.data
    });
  } catch (error) {
    res.status(500).json({
      message: 'Profile service test failed',
      error: error.message,
      response: error.response?.data || 'No response data'
    });
  }
  ...
  res.json({
    ...
    endpoints: {
      ...
      test: {
        ...
        profile_test: 'POST /test/profile'
      }
    }
  });
});
...
app.listen(port, () => {
  ...
  console.log(`   curl -X POST http://localhost:${port}/test/profile`);
});